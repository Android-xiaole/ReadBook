package com.jj.base.utils.toast;
...
/**
 * @link https://blog.csdn.net/weixin_34198453/article/details/86829316
 * 此自定义Toast可解决用户关闭通知不显示的问题
 */
public class CustomToast implements IToast {
    ...
    public static IToast makeText(Context context, CharSequence text, int duration) {
        return new CustomToast(context).setText(text).setDuration(duration)
                .setGravity(Gravity.BOTTOM, 0, CommonUtil.dip2px(context, 64));
    }
    ...
    public IToast setText(@StringRes int resId) {
        String text = BaseApplication.getApplication().getString(resId);
        View view = Toast.makeText(mContext, text, Toast.LENGTH_SHORT).getView();
        if (view != null) {
            TextView tv = view.findViewById(android.R.id.message);
            tv.setText(text);
